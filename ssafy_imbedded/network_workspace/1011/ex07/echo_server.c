...
int main(int argc, char *argv[]) {
    ...
    server_sock = socket(AF_INET, SOCK_STREAM, 0);
		// server_sock = -1; //socket() fail test
    if (server_sock == -1) {
    ...
    while (1) {
        ...
        // client_sock = -1; // accept() fail test
				if (client_sock == -1) {
        ...
        while (1) {                  
            ...
            removeEnterChar(write_buf);           
            write(client_sock, write_buf, strlen(write_buf));
