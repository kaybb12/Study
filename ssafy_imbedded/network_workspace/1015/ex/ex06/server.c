...
KeyValue key_value_store[MAX_KEY_VALUE_CNT];
int kv_count = 0;
...
void *client_handler(void *arg){
    ...
    while (1) {
        ...
        int num_tokens = sscanf(buf, "%s %s %s", command, key, value);
        
        //printf("%s %s %s %s",buf, command, key, value);
        if (num_tokens < 2) {
          char response[] = "Invalid input format\n";
          write(client_sock[id], response, strlen(response));
        ...
        if (strcmp(command, "save") == 0) {
          if (num_tokens < 3) {
            char response[] = "Invalid save format. Expected: save [KEY] [VALUE]\n";
            write(client_sock[id], response, strlen(response));
            pthread_mutex_unlock(&mlock);
            continue;
          }
          ...
          for (int i = 0; i < kv_count; ++i) {
            if (strcmp(key_value_store[i].key, key) == 0) {
              strcpy(key_value_store[i].value, value);
              found = 1;
              break;
            }
          }

          if (!found && kv_count < MAX_KEY_VALUE_CNT) {
            strcpy(key_value_store[kv_count].key, key);
            strcpy(key_value_store[kv_count].value, value);
            kv_count++;
          }

          char response[] = "Save successful";
          write(client_sock[id], response, strlen(response));
          ...
          for (int i = 0; i < kv_count; ++i) {
            if (strcmp(key_value_store[i].key, key) == 0) {
              ...
              sprintf(response, "Value: %s", key_value_store[i].value);
              write(client_sock[id], response, strlen(response));
              found = 1;
              break;
            }
          ...
          if (!found) {
            char response[] = "Error: Key not found";
            write(client_sock[id], response, strlen(response));
          }
        } else {
