...
char* encrypt(char* str) {
    ...
    memset(result, 0, sizeof(result));  // 결과 버퍼 초기화

    if (isdigit(str[0])) {  // 숫자 메시지 처리
        int num = atoi(str);
        sprintf(result, "%d", num * 2);  // 숫자를 두 배로
    } else {  // 문자 메시지 처리 (시저 암호)
        for (int i = 0; i < strlen(str); ++i) {
            if (isalpha(str[i])) {  // 문자인 경우만 처리
                if (islower(str[i])) {
                    result[i] = ((str[i] - 'a' + 3 + 26) % 26) + 'a';  // 소문자 처리
                } else {
                    result[i] = ((str[i] - 'A' + 3 + 26) % 26) + 'A';  // 대문자 처리
                }
            } else {
                result[i] = str[i];  // 알파벳이 아닌 문자는 그대로
            }
        }
    }
...
char* decrypt(char* str) {
    ...
    if (isdigit(str[0])) {  // 숫자 메시지 처리
        int num = atoi(str);
        sprintf(result, "%d", num / 2);  // 숫자를 절반으로
    } else {  // 문자 메시지 처리 (시저 암호 복호화)
        for (int i = 0; i < strlen(str); ++i) {
            if (isalpha(str[i])) {  // 문자인 경우만 처리
                if (islower(str[i])) {
                    result[i] = ((str[i] - 'a' - 3 + 26) % 26) + 'a';  // 소문자 처리
...
void *client_handler(void *arg){
    ...
    while (1) {
        ...
        int num = 0;
        while (token != NULL) {
          strcpy(cmd[num], token);
          token = strtok(NULL, " ");  // 다음 토큰을 추출
          //printf("%s\n",cmd[num]);
          ++num;
        }
        
        if (strcmp(cmd[1], "1") == 0) {
          char *encrypted_msg = encrypt(cmd[2]);
          //printf("%s\n",encrypted_msg);
          strcat(buf, " ");
          strcat(buf, encrypted_msg);
          if (client_sock[id] != 0)      
            write(client_sock[id], buf, strlen(buf));
        }
        else if (strcmp(cmd[1], "2") == 0) {
          char *decrypted_msg = decrypt(cmd[2]);
          //printf("%s\n",decrypted_msg);
          strcat(buf, " ");
          strcat(buf, decrypted_msg);
          if (client_sock[id] != 0)      
            write(client_sock[id], buf, strlen(buf));
        }
        else {
